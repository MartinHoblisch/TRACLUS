...
```
```{r clustered_segs-plot}
plot_traclus_result(clustered_segs, title = "Clustering result (eps=10, min_lns=2)")
```
...
```{r representative}
phase3 <- compute_all_representatives(clustered_segs,
                                      min_lns = 2,
                                      gamma   = 0.5)
cat("Clusters with a representative:", length(phase3$representatives), "\n")

# You can also compute the representative for a single cluster directly:
single_cluster <- phase3$segments[!is.na(phase3$segments$cluster_id) &
                                    phase3$segments$cluster_id == 1, ]
rt <- compute_representative(single_cluster, min_lns = 3, gamma = 1)
head(rt)
```
```{r clustering-plot}
toy_phase3 <- compute_all_representatives(clustered_segs,
                                          min_lns = 2,
                                          gamma   = 0.5)
plot_traclus_result(toy_phase3$segments, toy_phase3$representatives,
                    title = "Clustering result (eps=10, min_lns=2)")
```
...
common pattern, `min_lns = 2` is a natural starting point.
...
hurdat_phase3 <- compute_all_representatives(hurdat_clustered,
                                             min_lns = 5,
                                             gamma   = 200)
cat("Representative trajectories:", length(hurdat_phase3$representatives), "\n")
```

### Visualisation

```{r hurdat-plot-clusters, fig.width = 9, fig.height = 6}
plot_traclus_result(hurdat_phase3$segments, hurdat_phase3$representatives,
                    mode    = "clusters",
...
cluster_ids <- sort(unique(hurdat_phase3$segments$cluster_id[
  !is.na(hurdat_phase3$segments$cluster_id)]))

m <- leaflet() |> addProviderTiles(providers$CartoDB.Positron)

# Noise segments in grey, dashed
noise_h <- hurdat_phase3$segments[is.na(hurdat_phase3$segments$cluster_id), ]
if (nrow(noise_h) > 0L) {
...
for (k in seq_along(cluster_ids)) {
  cid   <- cluster_ids[k]
  csub  <- hurdat_phase3$segments[!is.na(hurdat_phase3$segments$cluster_id) &
                              hurdat_phase3$segments$cluster_id == cid, ]
...
for (k in seq_along(hurdat_phase3$representatives)) {
  rt    <- hurdat_phase3$representatives[[k]]
  col_k <- pal[(k - 1L) %% length(pal) + 1L]
  ...
  m <- m |> addPolylines(data = line_sf,
                         ...
                         popup = paste("Cluster", names(hurdat_phase3$representatives)[k]))
}

# Legend matching cluster colours
m <- m |> addLegend(
  position = "bottomright",
  colors   = c("#999999", pal[seq_along(hurdat_phase3$representatives)]),
  labels   = c("Noise", paste("Cluster", names(hurdat_phase3$representatives))),
  title    = "Clusters",
  opacity  = 1
)
